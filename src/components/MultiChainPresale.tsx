...
export const MultiChainPresale: FC = () => {
  ...
  const getProgram = useCallback(() => {
    ...
    const programId = new PublicKey("CZzeem1owGnHwuq9AQXz4Q8ZWJcLhCziKSCVhXSdwjN2");
  ...
  const handleSolanaBuy = async (amount: number, referrer?: string) => {
    try {
      ...
      const transactionRecordData = await program.account.transactionRecord.fetch(transactionRecord);
      const existingBuyer = transactionRecordData.buyers.find(
        (buyer: any) => buyer.buyerAddress.equals(solanaPublicKey)
      );
    ...
    <div className={styles.presaleContainer}>
      ...
      >
        ...
        <div className={styles.walletSection}>
          ...
          {/* Admin controls */}
          {solanaConnected && isInitialized && isAdmin && (
            ...
            {!userProfile && (
              <div className={styles.inputGroup}>
                ...
                <p className={styles.helperText}>
                  If someone referred you, enter their wallet address to support them
                </p>
              </div>
            )}
                    ...
                    <div className={styles.statValue}>
                      <span className={styles.walletAddress}>
                        {typeof userProfile.referrer === 'string'
                          ? `${userProfile.referrer.slice(0, 4)}...${userProfile.referrer.slice(-4)}`
                          : `${userProfile.referrer.toString().slice(0, 4)}...${userProfile.referrer.toString().slice(-4)}`}
                      </span>
                    </div>
                  </div>
                )}
